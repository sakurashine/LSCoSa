...
from data_processing.utils import  get_device, sample_gt, count_sliding_window, compute_imf_weights, metrics, logger, display_dataset, display_goundtruth, sliding_window, grouper, PCA_data
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for i in range(RUN):
        ...
        for epoch in range(args.epochs):
            ...
            # 训练一个epoch
            Encoder_Features, Label = train(train_loader, model, criterion, optimizer, epoch, args, N_CLASSES)
        ...
        SVM_GRID_PARAMS = [
                        {'kernel': ['rbf'], 'gamma': [1e-1, 1e-2, 1e-3],'C': [1, 10, 100, 1000]},
                           ...
                           {'kernel': ['poly'], 'degree': [3], 'gamma': [1e-1, 1e-2, 1e-3],'max_iter':[100000]}#
                        ]
        ...
        clf.fit(Encoder_Features, Label)
        ...
        for idx, (images,labels) in enumerate(test_loader):
            ...
            with torch.no_grad():
                _,feature = model(images.to(args.cuda))#(32*256)
                for num in range(feature.size(0)):
                    Dncoder_Features.append(np.array(feature[num].cpu().detach().numpy()))#保存编码器特征到列表里
                    Label2.append(np.array(labels[num].cpu().detach().numpy()))
        Dncoder_Features = np.array(Dncoder_Features)#(911,256)
        Label2 = np.array(Label2)#(911,)
        print("**********************************")
        print(Dncoder_Features.shape, Label2.shape)

        prediction = clf.predict(Dncoder_Features)
        end_time = datetime.datetime.now()
        print("$$$$$$$$$$$$$$$$$$$$$$$4")
        print(prediction.shape, Label2.shape)
        correct = (torch.from_numpy(prediction) == torch.from_numpy(Label2)).sum().item()
        log.info(correct)
        acc_meter.update(correct/prediction.shape[0])
        log.info(acc_meter)
    ...
    # print("Training spend time(s):{}".format((mid_time-start_time).seconds))
    # print("Inference spend time(s):{}".format((end_time-mid_time).seconds))
...
def train(train_loader, model, criterion, optimizer, epoch, args, N_CLASSES):
    ...
    for i, (images, target ) in enumerate(train_loader):
        ...
        # flat_emb = flat_emb.cuda()
        # flat_emb = nn.Linear(128 , 200)(flat_emb)
        for num in range(emb.size(0)):
                Encoder_Features.append(np.array(emb[num].cpu().detach().numpy()))#保存编码器特征到列表里
                Label.append(np.array(target[num].cpu().detach().numpy()))
        ...
        if i % args.print_freq == 0:
            progress.display(i)
    Encoder_Features = np.array(Encoder_Features)#(911,256)
    Label = np.array(Label)#(911,)
    print("##############################3")
    print(Encoder_Features.shape, Label.shape)
    return Encoder_Features, Label
